...
function weather() {
    ...
    $httpClient.get(info, async function (error, response, data) {
        if (error) {
            ...
            $notification.post("Country: 🇻🇳 Vietnam
            Đồng Nai City", icon + " " + Math.round(daily_mintemp) + " - " + Math.round(daily_maxtemp) + "  ☔️ " + (Number(daily_prec_chance) * 100).toFixed(1)+ "%", hour_summary);        }
    });
}