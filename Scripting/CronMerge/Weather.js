...
function weather() {
    ...
    $httpClient.get(info, async function (error, response, data) {
        if (error) {
            console.log(error);
            $notification.post("Dark Sky", lat_lon + '\nConnection error\n', error);
        } else {
            ...
            $notification.post("🇻🇳 Đồng Nai City", icon + " " + Math.round(daily_mintemp) + " - " + Math.round(daily_maxtemp) + "  ☔️ " + (Number(daily_prec_chance) * 100).toFixed(1)+ "%", "⚠️ " + hour_summary.replace(/\./g, ""));        }
    });
}